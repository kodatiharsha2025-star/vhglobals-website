...
import { NAV_LINKS, CALENDLY_URL } from '../data';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <button
            ...
            className="flex items-center gap-2.5 group focus:outline-none focus:ring-2 focus:ring-[#2596be] rounded-md px-1 cursor-pointer bg-transparent border-0 text-left p-0"
          >
            <VHLogo className="w-9 h-9 transition-transform group-hover:scale-105 duration-200" />
        ...
        >
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              ...
              >
                <span>{link.label}</span>
                {link.isExternal && <ArrowUpRight className="w-3.5 h-3.5 text-[#2596be]" />}
              </a>
            );
          })}
          <div className="pt-3">
            <a
              ...
              rel="noopener noreferrer"
              onClick={() => setMobileMenuOpen(false)}
              className="w-full flex items-center justify-center gap-2 py-3 rounded-lg text-sm font-semibold text-white bg-[#2596be] hover:bg-[#1f82a6] transition-colors shadow-sm"
            >
              <PhoneCall className="w-4 h-4" />
              <span>Book Consultation</span>
            </a>
